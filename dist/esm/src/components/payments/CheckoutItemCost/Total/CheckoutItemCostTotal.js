...
const CheckoutItemCostTotal = ({ total, taxes: { status, taxAmount = 0, taxRate = 0, }, fees, withDetails = false, }) => {
    ...
    if (status === "loading") {
        ...
        totalElement = React__default.createElement(Number, { suffix: " USD" }, total + fees + taxAmount);
    }
    else {
        taxRateElement = React__default.createElement(Tooltip, { title: "Enter a valid address to calculate the taxes" },
            React__default.createElement("span", null, "(- %)"));
        taxAmountElement = React__default.createElement(Tooltip, { title: "Enter a valid address to calculate the taxes" },
            React__default.createElement("span", null, "- USD"));
