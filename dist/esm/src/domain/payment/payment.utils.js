...
function standaloneGetCardImageProps(network) {
  ...
  var cardType = network.toLowerCase(); // See https://github.com/medipass/react-payment-inputs/blob/master/src/usePaymentInputs.js#L452
...
var getCardNumberIsValid = function getCardNumberIsValid(cardNumber) {
  ...
  if (cardType && cardType.lengths) {
    ...
    if (doesCardNumberMatchLength) {
      ...
      if (isLuhnValid) return true;
    }
  }

  return false;
};
var getExpiryDateIsvalid = function getExpiryDateIsvalid(expiryDate) {
  if (!expiryDate) return false;
  var rawExpiryDate = expiryDate.replace(" / ", "").replace("/", "");
  if (rawExpiryDate.length !== 4) return false;
  ...
  var dateIsOnValidRange = parseInt(year) !== new Date().getFullYear() || parseInt(month) >= new Date().getMonth() + 1;
  return monthIsOnValidRange && yearIsOnValidRange && dateIsOnValidRange;
};
var getCVCIsValid = function getCVCIsValid(cvc, cardNumber) {
  if (!cvc || cvc.length < 3) return false;
  var rawCardNumber = cardNumber.replace(/\s/g, "");
  var cardType = getCardTypeByValue(rawCardNumber);
  if (cardType && cvc.length !== cardType.code.length) return false;
  return true;
};
