...
import { __rest, __assign, __makeTemplateObject } from '../../../node_modules/tslib/tslib.es6.js';
import countryRegionData from '../../../node_modules/country-region-data/dist/data-umd.js';
import { customList } from 'country-codes-list';

var countryPrefixes = customList('countryCode', '{countryCallingCode}');
...
function transformRawSavedPaymentMethods(rawSavedPaymentMethods) {
  ...
  return rawSavedPaymentMethods.map(function (_a) {
    var billingDetails = _a.billingDetails,
        ...
        rest = __rest(_a, ["billingDetails", "metadata"]);

    if (!billingDetails || !metadata) return null; // Find country by short code:

    var _b = countryRegionData.find(function (_a) {
    ...
    var savedPaymentMethod = __assign(__assign(__assign({}, rest), savedPaymentInfoBillingInfo), {
...
function isCircleFieldError(obj) {
  return obj && _typeof(obj) === "object" && obj.hasOwnProperty("error") && obj.hasOwnProperty("invalidValue") && obj.hasOwnProperty("location") && obj.hasOwnProperty("message");
}
...
function parseCircleError(error) {
  ...
  if (message.includes("with body: ")) {
    try {
      var parsedCircleError = JSON.parse(String.raw(templateObject_1 || (templateObject_1 = __makeTemplateObject(["", ""], ["", ""])), message).replace(/^.+with body: /, ''));
