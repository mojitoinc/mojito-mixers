...
function addLightOrDark(intent, direction, shade, tonalOffset) {
  ...
  if (!intent[direction]) {
    if (intent.hasOwnProperty(shade)) {
      ...
      intent.light = system.lighten(intent.main, tonalOffsetLight);
    } else if (direction === 'dark') {
      intent.dark = system.darken(intent.main, tonalOffsetDark);
    }
  }
}
...
function createPalette(palette) {
  const {
    ...
    tonalOffset = 0.2
        ...
        other = _objectWithoutPropertiesLoose__default["default"](palette, _excluded);
  ...
  function getContrastText(background) {
    const contrastText = system.getContrastRatio(background, dark.text.primary) >= contrastThreshold ? dark.text.primary : light.text.primary;

    if (process.env.NODE_ENV !== 'production') {
      const contrast = system.getContrastRatio(background, contrastText);
...
const theme2 = createTheme({ palette: {
  ...
  const paletteOutput = deepmerge["default"](_extends__default["default"]({
